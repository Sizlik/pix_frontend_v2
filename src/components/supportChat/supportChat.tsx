...
export default function SupportChat() {
  ...
  const sendMessage = (event: FormEvent<HTMLFormElement>) => {
    ...
    socket?.send(
      JSON.stringify({ message: event.currentTarget.message.value })
    );
  ...
  useEffect(() => {
    if (isOpened) {
      ...
      //   `ws://localhost:8000/api_v1/chat/ws?auth=${getCookie("token")!.split(" ")[1]}`
      // );
      const newSocket = new WebSocket(
        `wss://pixlogistic.com/api_v1/chat/ws?auth=${getCookie("token")!.split(" ")[1]}`
      );
      setSocket(newSocket);
            ...
            <div className="h-full flex flex-col-reverse gap-2 py-2 overflow-y-auto scrollbar">
              {messages?.map((item, index) => {
                return (
                  <Message
                    key={index}
                    isSender={item.from_user_id == item.to_chat_room_id}
